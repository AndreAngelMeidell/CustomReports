...
LEFT JOIN [NGVRSDBCIMST01P].VBDCM.dbo.WorkAreaTransfers wat (NOLOCK)  ON wat.WorkAreaTransferNo=sc.StockCountNo AND sat.StockAdjustmentTypeNo IN (79)
LEFT JOIN [BI_Mart].RBIM.Dim_ReasonCode rc2 (NOLOCK) ON rc2.ReasonNo=wat.ReasonCodeNo
LEFT JOIN [NGVRSDBCIMST01P].VBDCM.dbo.Deliveries del (NOLOCK) ON del.DeliveryNoteNo=sc.StockCountNo AND sat.StockAdjustmentTypeNo IN (3)
