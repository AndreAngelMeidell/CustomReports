...
/****** Object:  StoredProcedure [dbo].[usp_CBI_GenusReportsSalesAccWeek]    Script Date: 26.03.2020 09:41:56 ******/
SET ANSI_NULLS ON
GO
...
--  20200325 Settes i normal drift
    ...
    BEGIN
		...
		(
			SELECT 
				...
				'N/A' as 'Grossist',
				article.Lev4ArticleHierarchyId as 'Varegruppe',
		...
		(
			...
			FROM 
				...
				LEFT JOIN BI_Mart.RBIM.Out_ArticleExtraInfo ArtExtIn on ArtExtIn.ArticleId = article.ArticleId
				left join BI_Mart.RBIM.Dim_Supplier supp on supp.SupplierIdx = sales.SupplierIdx
				--INNER JOIN BI_Mart.RBIM.Agg_CampaignSalesPerHour campaign ON 
				--	sales.ReceiptDateIdx = campaign.ReceiptDateIdx 
				--	AND sales.StoreIdx = campaign.StoreIdx 
				--	AND sales.ArticleIdx = campaign.ArticleIdx
				--	AND sales.SupplierIdx = campaign.SupplierIdx
				--	--and sales.SalesAmount= campaign.SalesAmount
				--JOIN bi_mart.RBIM.Dim_PriceType AS DPT ON DPT.PriceTypeIdx = campaign.PriceTypeIdx
			WHERE 
				tdate.WeekNumberOfYear = DATEPART(WEEK,@InParamDate) AND tdate.Year = DATEPART(YEAR,@InParamDate) AND tdate.FullDate IS NOT NULL
				...
				AND store.GlobalLocationNo = @InParamGln
				AND sales.ArticleIdx <>-4
				--and sales.PriceTypeIdx <> 19 -- ikke ta med overføring mellom butikk
		)
		,ovun as
		(
			select 'OVUN' as 'Rapport_Kode'
			, Filial
			, Dato
			, AID_Kode
			, Nummer
