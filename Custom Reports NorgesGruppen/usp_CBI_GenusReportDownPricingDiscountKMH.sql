...
/****** Object:  StoredProcedure [dbo].[usp_CBI_GenusReportDownPricingDiscountKMH]    Script Date: 04.06.2020 13:34:33 ******/
SET ANSI_NULLS ON
GO
...
--
--  Modifications:
--                  20190301 Andre Meidell lagt til "AND (f.WeighedSalesAmount-f.WeighedReturnAmount)<>0"
--                  20190311 Andre Meidell endret til AND (f.WeighedSalesAmount)<>0     
--                  20190525 fra--Where ('+CAST(dbo.GetChainGroupNoJoker() AS varchar)+' in (ds.Lev1ChainGroupNo,ds.Lev2ChainGroupNo,ds.Lev3ChainGroupNo,ds.Lev4ChainGroupNo,ds.Lev5ChainGroupNo)
--                     --OR '+CAST(dbo.GetChainGroupNoSpar() AS varchar)+' in (ds.Lev1ChainGroupNo,ds.Lev2ChainGroupNo,ds.Lev3ChainGroupNo,ds.Lev4ChainGroupNo,ds.Lev5ChainGroupNo)
--                     --OR ds.StoreIdx<0)
--til:
--Where
--          (
--              ' + CAST(dbo.GetChainGroupNoJoker() AS varchar) + ' in (
...
--              OR
--              ' + CAST(dbo.GetChainGroupNoSpar() AS varchar) + ' in (
--              ds.Lev1chainGroupNo,ds.Lev2chainGroupNo,ds.Lev3chainGroupNo,ds.Lev4chainGroupNo,
--              ds.Lev1RegionGroupNo,ds.Lev2RegionGroupExternalId,ds.Lev3RegionGroupNo,ds.Lev4RegionGroupNo,ds.Lev5RegionGroupNo,
--              ds.Lev1LegalGroupNo,ds.Lev2LegalGroupNo,ds.Lev3LegalGroupNo,ds.Lev4LegalGroupNo,ds.Lev5LegalGroupNo,
--              ds.AuthorizationRegionGroupId,ds.AuthorizationLegalGroupId)
...
----                  
--
CREATE PROCEDURE [dbo].[usp_CBI_GenusReportDownPricingDiscountKMH]
(
...
BEGIN
    ...
    DECLARE @groupName VARCHAR(50) = 'KMH'
...
--SET @Folder = 'd:\genusftp\backup\'
    ...
    SELECT @DateFromIdx
    SET @sqlStr =
'SELECT CONVERT(VARCHAR(10),dd.FullDate,104),
gt.Gtin,
da.ArticleName,
...
ds.GlobalLocationNo
from BI_Mart.RBIM.Agg_SalesAndReturnPerDay f with (nolock)
...
group by da.ArticleName, E.PriceTypeNo, E.IsArtsPriceType, gt.gtin, ds.GlobalLocationNo, dd.FullDate, f.WeighedSalesAmount '
