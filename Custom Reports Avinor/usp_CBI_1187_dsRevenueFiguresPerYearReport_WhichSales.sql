...
/****** Object:  StoredProcedure [dbo].[usp_CBI_1187_dsRevenueFiguresPerYearReport_WhichSales]    Script Date: 14.09.2018 13:19:41 ******/
SET ANSI_NULLS ON
GO
...
SELECT 
		...
		APACOL.ReceivedQty*APACOL.ArticleDeliveredPrice AS Revenue,
...
(
	SELECT 
		...
		NoOfCustomers = ( SELECT COUNT(*) FROM CBIM.Agg_PickAndCollectOrders AS APACO (nolock) --P&C
						INNER JOIN RBIM.Dim_Date AS DD ON (convert(date,APACO.PaymentSuccessTimeStamp,103)) = dd.FullDate
						JOIN @stores ds  ON ds.StoreId = APACO.StoreId
						WHERE (dd.DateIdx BETWEEN @DateFromIdx AND @DateToIdx)) -- legges til i de andre dato uttrekk
...
(
	SELECT 
		...
		NoOfCustomers = ( SELECT COUNT(*) FROM CBIM.Agg_PickAndCollectOrders AS APACO (nolock) --P&C
						INNER JOIN RBIM.Dim_Date AS DD ON (convert(date,APACO.PaymentSuccessTimeStamp,103)) = dd.FullDate
						JOIN @stores ds  ON ds.StoreId = APACO.StoreId
						WHERE (dd.DateIdx BETWEEN @DateFromIdxLastYear AND @DateToIdxLastYear)) -- legges til i de andre dato uttrekk
	FROM SalesPC--SalesReceipts
	WHERE ReceiptDateIdx BETWEEN @DateFromIdxLastYear AND @DateToIdxLastYear						
...
(
	SELECT 
		...
		NoOfCustomers = ( SELECT COUNT(*) FROM CBIM.Agg_PickAndCollectOrders AS APACO (nolock) --P&C
						INNER JOIN RBIM.Dim_Date AS DD ON (convert(date,APACO.PaymentSuccessTimeStamp,103)) = dd.FullDate
						JOIN @stores ds  ON ds.StoreId = APACO.StoreId
						WHERE (dd.DateIdx BETWEEN @DateFromIdxStartThisYear AND @DateToIdx	)) -- legges til i de andre dato uttrekk
	FROM SalesPC--SalesReceipts
	WHERE ReceiptDateIdx BETWEEN @DateFromIdxStartThisYear AND @DateToIdx						
...
(
	SELECT 
		...
		NoOfCustomers = ( SELECT COUNT(*) FROM CBIM.Agg_PickAndCollectOrders AS APACO (nolock) --P&C
						INNER JOIN RBIM.Dim_Date AS DD ON (convert(date,APACO.PaymentSuccessTimeStamp,103)) = dd.FullDate
						JOIN @stores ds  ON ds.StoreId = APACO.StoreId
						WHERE (dd.DateIdx BETWEEN @DateFromIdxStartLastYear AND @DateToIdxLastYear	)) -- legges til i de andre dato uttrekk
	FROM SalesPC--SalesReceipts
	WHERE ReceiptDateIdx BETWEEN @DateFromIdxStartLastYear AND @DateToIdxLastYear						
