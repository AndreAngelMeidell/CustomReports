...
/****** Object:  StoredProcedure [dbo].[usp_CBI_1211_Weekly_Sales_Report_Chain]    Script Date: 27.04.2021 08:30:24 ******/
DROP PROCEDURE [dbo].[usp_CBI_1211_Weekly_Sales_Report_Chain]
GO
...
CREATE PROCEDURE [dbo].[usp_CBI_1211_Weekly_Sales_Report_Chain]
(
	---------------------------------------------
--	@Week as int,
	@District as int
...
CurrentWeek AS
(SELECT DS.StoreName, SUM(ASARPW.SALESREVENUE) AS 'OMSÄTTNING', SUM(ASARPW.GrossProfit) AS 'MARGINAL', SUM(ASARPW.NumberOfCustomers) AS 'ANTAL KUNDER', (SUM(ASARPW.SALESREVENUE)/SUM(ASARPW.NumberOfCustomers)) AS 'SNITTKÖP', SUM(ASARPW.QuantityOfArticlesSold) AS 'ANTAL ARTIKLAR'
...
Select substring(CurrentWeek.StoreName,7,50) AS 'BUTIK',
		...
		from CurrentWeek
JOIN PreviousWeek AS PWeek ON CurrentWeek.StoreName = PWeek.StoreName
ORDER BY CurrentWeek.StoreName
