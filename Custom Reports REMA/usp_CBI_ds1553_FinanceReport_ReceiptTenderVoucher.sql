...
/****** Object:  StoredProcedure [dbo].[usp_CBI_ds1553_FinanceReport_ReceiptTenderVoucher]    Script Date: 07.02.2020 10:41:34 ******/
SET ANSI_NULLS ON
GO
...
CREATE   Procedure [dbo].[usp_CBI_ds1553_FinanceReport_ReceiptTenderVoucher]
...
OUTPUT
		...
		--Start Create temp Fact_ReceiptTender
		...
		--END Creat temp Fact_ReceiptTender
		...
		--		 td.TenderName,	
		--		 tc.DefaultSign,
		--		 'Vouchers' as TenderSelectionName
		...
		--  where ts.TenderSelectionId = @VoucherTenderSelectionId
		--),
		DROP TABLE IF EXISTS #TenderSelectionVouchers;
		...
		( select  ts.TenderName as VoucherType
				 ,count(*) as VoucherCount
				 --,sum(fc.[Amount]*isnull(nullif(fc.ExchangeRateToLocalCurrency,0.0),1.0)*ts.DefaultSign) as VoucherAmount --{RS-37082}
				 ,sum(fc.[Amount]) as VoucherAmount 
		  from #FRT (nolock) fc
			   ...
			   inner join #TenderSelectionVouchers ts on fc.TenderIdx = ts.TenderIdx 
