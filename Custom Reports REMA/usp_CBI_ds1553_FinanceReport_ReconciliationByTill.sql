...
/****** Object:  StoredProcedure [dbo].[usp_CBI_ds1553_FinanceReport_ReconciliationByTill]    Script Date: 07.02.2020 10:42:07 ******/
SET ANSI_NULLS ON
GO
...
CREATE   Procedure [dbo].[usp_CBI_ds1553_FinanceReport_ReconciliationByTill]
(
...
begin
	...
	IF (@ReconciliationType <> 4)		-- Check if this procedure is needed for the report.
	...
	-----------------------------------------
		...
		(	select  'POS' AS SourceType,ds.Lev1RegionGroupDisplayId,ds.Lev2RegionGroupDisplayId,ds.Lev3RegionGroupDisplayId,ds.Lev4RegionGroupDisplayId,ds.Lev5RegionGroupDisplayId, ds.CurrentStoreName as StoreName,ds.StoreId,  --{RS-36137}
			...
			where dt.TotalTypeId=4 /*dt.TotalTypeName='Till totaler'*/
			...
			group by ds.Lev1RegionGroupDisplayId,ds.Lev2RegionGroupDisplayId,ds.Lev3RegionGroupDisplayId,ds.Lev4RegionGroupDisplayId,ds.Lev5RegionGroupDisplayId, ds.CurrentStoreName,ds.StoreId, --{RS-36137}
					 ...
					 fc.ZNR, fc.TillId, ts.TenderSelectionName, du.[LoginName] , du.[FirstName], du.[LastName]
		),
		...
		( select  'POS' AS SourceType,ds.Lev1RegionGroupDisplayId,ds.Lev2RegionGroupDisplayId,ds.Lev3RegionGroupDisplayId,ds.Lev4RegionGroupDisplayId,ds.Lev5RegionGroupDisplayId, ds.CurrentStoreName as StoreName,ds.StoreId, --{RS-36137}
		  ...
		  group by ds.Lev1RegionGroupDisplayId,ds.Lev2RegionGroupDisplayId,ds.Lev3RegionGroupDisplayId,ds.Lev4RegionGroupDisplayId,ds.Lev5RegionGroupDisplayId, ds.CurrentStoreName,ds.StoreId, --{RS-36137}
				   ds.Lev1RegionGroupNo,ds.Lev2RegionGroupNo,ds.Lev3RegionGroupNo,ds.Lev4RegionGroupNo,ds.Lev5RegionGroupNo, ds.NumOfRegionLevels,
				   fc.ZNR, fc.TillId, da.AccumulationTypeSelectionName, du.LoginName, du.[FirstName], du.[LastName]
		),
		...
		( select  'POS' AS SourceType,ds.Lev1RegionGroupDisplayId,ds.Lev2RegionGroupDisplayId,ds.Lev3RegionGroupDisplayId,ds.Lev4RegionGroupDisplayId,ds.Lev5RegionGroupDisplayId, ds.CurrentStoreName AS StoreName,ds.StoreId, --{RS-36137}
				 ...
				 fc.TenderIdx, 
				 fc.CurrencyIdx, 
				 fc.Amount as CountedCashAmount
				 --(fc.Amount*isnull(fc.Rate/nullif(cast(fc.Unit as decimal(19,5)),0),1)*ts.DefaultSign) as CountedCashAmount,   -- {RS-37082} 
				 --row_number() over (partition by fc.StoreIdx, fc.ZNR, fc.TenderIdx, fc.CurrencyIdx order by CountNo desc) as ReverseCountNo 
		  from [RBIM].[Fact_ReconciliationCountingPerTender] (nolock) fc
			   --inner join [RBIM].[Fact_ReconciliationSystemTotalPerTender] (nolock) rec on fc.[StoreIdx]=rec.[StoreIdx] and fc.[ZNR]=rec.[ZNR] and fc.[TotalTypeIdx]=rec.[TotalTypeIdx]
			   	 inner join 	
		...
		( select 'CIM' AS SourceType, ds.Lev1RegionGroupDisplayId,ds.Lev2RegionGroupDisplayId,ds.Lev3RegionGroupDisplayId,ds.Lev4RegionGroupDisplayId,ds.Lev5RegionGroupDisplayId, ds.CurrentStoreName AS StoreName,ds.StoreId,  --{RS-36137}
				 ds.Lev1RegionGroupNo,ds.Lev2RegionGroupNo,ds.Lev3RegionGroupNo,ds.Lev4RegionGroupNo,ds.Lev5RegionGroupNo,
				 NULL AS ZNR, NULL AS [TillId],  du.[LoginName] as CashierLogin, 
 				 (du.[FirstName] + du.[LastName]) as CashierName, 
		  ...
		  group by ds.Lev1RegionGroupDisplayId,ds.Lev2RegionGroupDisplayId,ds.Lev3RegionGroupDisplayId,ds.Lev4RegionGroupDisplayId,ds.Lev5RegionGroupDisplayId, ds.CurrentStoreName,ds.StoreId,  --{RS-36137}
				   ds.Lev1RegionGroupNo,ds.Lev2RegionGroupNo,ds.Lev3RegionGroupNo,ds.Lev4RegionGroupNo,ds.Lev5RegionGroupNo,
				   dc.CashRegisterId, dc.CashRegisterName,ds.NumOfRegionLevels, du.[LoginName] ,  (du.[FirstName] + du.[LastName])
		)
		SELECT * FROM (
		...
		select  isnull(pt.SourceType,isnull(pa.SourceType,pc.SourceType)) as SourceType,
				...
				isnull(pt.TillId,pa.TillId) as TillId, 
