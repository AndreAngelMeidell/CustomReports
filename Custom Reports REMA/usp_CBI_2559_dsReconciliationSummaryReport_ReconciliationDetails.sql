...
/****** Object:  StoredProcedure [dbo].[usp_CBI_2559_dsReconciliationSummaryReport_ReconciliationDetails]    Script Date: 07.02.2020 10:33:55 ******/
SET ANSI_NULLS ON
GO
...
CREATE   PROCEDURE [dbo].[usp_CBI_2559_dsReconciliationSummaryReport_ReconciliationDetails]     
(
@TotalTypeId AS INT ,	
---------------------------------------------
...
--20200207 Endrer til Agg_ReceiptTenderDaily
...
SET @TotalTypeId = 2 --RS-45270
	...
	ELSE BEGIN	
		...
		DROP TABLE IF EXISTS #KeyFiguresReceiptTender
			...
			FROM RBIM.Agg_ReceiptTenderDaily f
			INNER JOIN #TenderSelection dt ON dt.TenderIdx = f.TenderIdx
		...
		--With this 20200207 AM
		    DROP TABLE IF EXISTS #KeyFiguresSalesAndReturn
			SELECT 
					asr.StoreIdx					
					,SUM(asr.BottleDepositReturnAmount) AS TotalBottleDeposit 
			INTO #KeyFiguresSalesAndReturn
			FROM RBIM.Agg_CashierSalesAndReturnPerReceipt as asr  			
			INNER JOIN #Stores ds ON ds.StoreIdx = asr.StoreIdx								 		
			WHERE
			(((asr.ReceiptDateIdx = @ReportReconciliationDateFromIdx AND asr.ReceiptTimeIdx >= @ReportReconciliationTimeFromIdx) 
			OR (asr.ReceiptDateIdx = @ReportReconciliationDateToIdx AND asr.ReceiptTimeIdx <= @ReportReconciliationTimeToIdx))
