...
/****** Object:  StoredProcedure [dbo].[usp_RBI_getArtsTenderInfo]    Script Date: 27.05.2021 13:41:57 ******/
DROP PROCEDURE [dbo].[usp_RBI_getArtsTenderInfo]
GO
...
------------------------------------------------------
         ...
         ----------------  THIS IS < 17.2 POS STANDARD TENDERS
		   ...
		   WHEN R.TenderType = 'AccountsPayable' 
				THEN '34'
--24.02.2021 Dag Erik Lund added custom handling for lotteryticket prize. Added after upgrade to 17.44 Andre
           WHEN R.TenderType = 'CashPrize'
                THEN '520'
...
-----------------------------------------------------------
           ...
           WHEN R.InternalTenderId IS NOT NULL AND R.InternalTenderId >= 100 THEN TTT.[TransactionTypeId]
		    ------------------ THIS IS 17.2 POS STANDARD TENDERS
		   WHEN R.IsDeposit = 1 AND LOWER(R.DepositResult) = 'OK'  
				THEN 30111 
           WHEN R.IsDeposit = 1 AND LOWER(R.DepositResult) = 'FlagError'  
				THEN 30112 
           WHEN R.IsDeposit = 1 AND LOWER(R.DepositResult) = 'Error'  
				THEN 30113 	
           WHEN R.IsWithDrawal = 1 AND LOWER(R.DepositResult) = 'OK'  
				THEN 30110 
           WHEN R.IsWithDrawal = 1 AND LOWER(R.DepositResult) = 'NoCash'  
				THEN 30114 
           WHEN R.IsWithDrawal = 1 AND LOWER(R.DepositResult) = 'FlagError'  
				THEN 30115 
           WHEN R.IsWithDrawal = 1 AND LOWER(R.DepositResult) = 'Error'  
				THEN 30116 		
