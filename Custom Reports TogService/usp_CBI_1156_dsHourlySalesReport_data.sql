...
if @Comparing = 0 
	...
	-----------------------
		...
		(   select 
				...
				,SUM(f.[QuantityOfArticlesSold])						as QuantityOfArticlesSoldPeriod1
				...
				,SUM(f.NumberOfArticlesSold)							as NumberOfArticlesSoldPeriod1
			 ...
			 from(
				select distinct dt.hour, f.*	                                                           
				from BI_Mart.RBIM.Agg_SalesAndReturnPerHour f 
				join rbim.Dim_Date       dd on dd.DateIdx    = f.ReceiptDateIdx 
				join rbim.Dim_Time       dt on dt.TimeIdx    = f.ReceiptTimeIdx
...
,SelectedSales as
	...
	select   sub.[Hour]
			...
			,case when Convert(decimal(19,5), SUM(NumberOfCustomersPeriod1)) = 0 then 0 else round(SUM(NumberOfArticlesSoldPeriod1)/Convert(decimal(19,5), SUM(NumberOfCustomersPeriod1)),2) end AS AvgArticlesSoldPerCustomerPeriod1
			,case when Convert(decimal(19,5), SUM(NumberOfCustomersPeriod2)) = 0 then 0 else round(SUM(NumberOfArticlesSoldPeriod2)/Convert(decimal(19,5), SUM(NumberOfCustomersPeriod2)),2) end AS AvgArticlesSoldPerCustomerPeriod2 
	...
	from 
		...
		(select 
				...
				,CASE WHEN @ArticleIdFrom IS NOT NULL OR @ArticleIdTo IS NOT NULL OR @GtinFrom IS NOT NULL OR @GtinTo IS NOT NULL THEN SUM(f.[NumberOfCustomersPerSelectedArticle]) ELSE SUM(f.[NumberOfCustomers]) END AS NumberOfCustomersPeriod1
				,0 as NumberOfCustomersPeriod2
				,SUM(f.[QuantityOfArticlesSold]) as QuantityOfArticlesSoldPeriod1 -- antall varer, ikke distinct
				,0 as QuantityOfArticlesSoldPeriod2
				,SUM(f.[SalesAmountExclVat] + f.[ReturnAmountExclVat])  as SalesRevenueExclVatPeriod1
				,0 as SalesRevenueExclVatPeriod2
			...
			FROM (
				...
				FROM BI_Mart.RBIM.Agg_SalesAndReturnPerHour f 
					...
					join Stores ds on ds.StoreIdx = f.StoreIdx
				WHERE
						/* no longer using join with Cov_ArticleGtin
		...
		select 
				...
				,SUM(f.[QuantityOfArticlesSold]) as QuantityOfArticlesSoldPeriod2
				...
				,SUM(f.NumberOfArticlesSold) AS NumberOfArticlesSoldPeriod2
