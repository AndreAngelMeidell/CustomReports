...
/****** Object:  StoredProcedure [dbo].[usp_RBI_getErrArtsTenderInfo]    Script Date: 24.02.2021 11:51:36 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[usp_RBI_getErrArtsTenderInfo] (@p1 bigint,@p2 bigint)
AS
BEGIN
 ...
 ------------------------------
         ...
         ISNULL(NULLIF(RTRIM(LTRIM(H.AssociateId)),''),'N/A') AS UserName,
...
-----------------------------------------------------------
  FROM [RBIS].[tmpArtsXmlReceiptHead] H WITH(NOLOCK)
    INNER JOIN [RBIS].[tmpArtsErrTenderReceipts] tmp WITH(NOLOCK)
	ON tmp.sArtsXmlReceiptHeadIdx = H.sArtsXmlReceiptHeadIdx
  INNER JOIN [RBIS].[tmpArtsXmlReceiptRow] R WITH(NOLOCK)
    ON R.sArtsXmlReceiptHeadIdx = tmp.sArtsXmlReceiptHeadIdx
...
------ TRANSACTION MAPPING - ONLY CUSTOM TENDERS 
  LEFT OUTER JOIN  [RBIS].[Lcp_TenderTransactionType] AS TTT WITH(NOLOCK) 
    ON CASE	
...
----------------------------------
  WHERE H.RowIdx BETWEEN @StartRowIdx AND @StopRowIdx 
  AND H.sArtsXmlReceiptHeadIdx IN (SELECT sartsxmlreceiptheadidx FROM RBIS.tmpArtsErrTenderReceipts WITH(NOLOCK)) 
