...
/****** Object:  StoredProcedure [dbo].[usp_RBI_0559_dsReconciliationSummaryReport_BagIdDetails]    Script Date: 14.09.2017 14:10:10 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[usp_CBI_1560_dsReconciliationSummaryReport_BagIdDetails]     
(
@TotalTypeId AS INT ,	
	---------------------------------------------
...
AS  
BEGIN 
-- 14.09.2017: Taken from usp_RBI_0559_dsReconciliationSummaryReport_BagIdDetails and added daterange instead of date.
...
ReconciliationDateZnr AS( -- Get all ZNRs that has selected reconciliation date 
  SELECT DISTINCT ZNR FROM RBIM.Fact_ReconciliationSystemTotalPerTender st
  ...
  WHERE ds.StoreId = @StoreId AND (dd.FullDate BETWEEN @DateFrom AND @DateTo) AND dtt.TotalTypeId = @TotalTypeId AND dt.TenderId in ('1','8') AND ds.IsCurrentStore = 1
)
