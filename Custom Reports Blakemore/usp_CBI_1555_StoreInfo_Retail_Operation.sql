...
/****** Object:  StoredProcedure [dbo].[usp_CBI_1555_StoreInfo]    Script Date: 23.08.2019 11:46:45 ******/
SET ANSI_NULLS ON
GO
...
,S.StoreStatus
INTO #Store_Flattern
FROM [AFBS-VRSSQL-R1].RSItemESDb.dbo.StoreInfos AS SI
LEFT JOIN [AFBS-VRSSQL-R1].RSItemESDb.dbo.Stores AS S ON S.StoreNo = SI.StoreNo
...
INTO #StoreFormat
FROM [AFBS-VRSSQL-R1].RSCompanyAdminESDb.dbo.Stores AS S 
JOIN [AFBS-VRSSQL-R1].RSCompanyAdminESDb.dbo.StoreGroupLinks AS SGL ON SGL.StoreNo = S.StoreNo
JOIN [AFBS-VRSSQL-R1].RSCompanyAdminESDb.dbo.StoreGroups AS SG ON SG.StoreGroupNo = SGL.StoreGroupNo AND SG.StoreGroupTypeNo = 14
WHERE 1=1
--AND s.StoreID=14777 
...
JOIN [AFBS-VRSSQL-R1].RSItemESDb.dbo.Stores AS S ON S.StoreID = DS.StoreId
JOIN #Store_Flattern AS SF ON SF.StoreID = DS.StoreId
JOIN [AFBS-VRSSQL-R1].RSItemESDb.dbo.StoreGroupStates AS SGS ON SGS.StoreGroupStatus=SF.StoreStatus
